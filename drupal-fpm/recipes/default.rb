...
node[:hosts][:localhost_aliases].each do |site|
  ...
  nginx_app site do
    ...
    server_name site
    server_aliases [site]
    docroot "/var/www/virtual/#{site}/www"
  end
end
...
execute "add-admin-user" do
  command "/usr/bin/mysql -u root -p#{node[:mysql][:server_root_password]} -e \"" +
      "CREATE USER 'localmysqladmin'@'localhost' IDENTIFIED BY 'P87pXw6jAUasI6r';" +
      "GRANT ALL PRIVILEGES ON *.* TO 'localmysqladmin'@'localhost' WITH GRANT OPTION;" +
      "CREATE USER 'localmysqladmin'@'%' IDENTIFIED BY 'P87pXw6jAUasI6r';" +
      "GRANT ALL PRIVILEGES ON *.* TO 'localmysqladmin'@'%' WITH GRANT OPTION;\" " +
      "mysql"
  action :run
  ignore_failure true
end
